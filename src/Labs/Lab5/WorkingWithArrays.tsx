...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <br/><br/>
            <hr/>
            <a id="wd-retrieve-completed-todos" target="blank" href={`${API}/${todo.id}/completed/${todo.completed}`}
               className="btn btn-primary float-end">
                Update Todo Completed Status</a>
            <input value={todo.id} className="form-control w-25 float-start me-2"
                   onChange={(e) => setTodo({...todo, id: e.target.value})}
            />
            <label htmlFor="wd-retrieve-completed-todos">
                Completed:{" "}
                <input type="checkbox" checked={todo.completed}
                       onChange={(e) => setTodo({...todo, completed: Boolean(e.target.checked)})}/>
            </label>
            <br/>
            <br/>
            <a id="wd-retrieve-completed-todos" target="blank"
